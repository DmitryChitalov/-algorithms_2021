...
class PlateStackClass:
    ...
    def push_in(self, el):# Вставка
        """Предполагаем, что верхний элемент стека находится в конце списка
        если размер стопки равен пороговому значению то создается новая стопка
        и туда кладется значние"""
        if len(self.elems[len(self.elems)-1]) < self.max_size: # Если превышаем максим. значение, то создаем новую вставку
            self.elems[len(self.elems)-1].append(el)
        else:
            self.elems.append([]) # Создание новой вставки через создание нового массива
            self.elems[len(self.elems) - 1].append(el)

    def pop_out(self): # Удаление
    ...
    def get_val(self):# Получение вершины стека
    ...
    def stack_count(self): # Общее количество стопок
