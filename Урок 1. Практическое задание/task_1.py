...
def check_1(lst_obj):
    ...
    return lst_to_set  # O(N) - линейная


# итоговая сложность O(N) - линейная так как максимальная сложность O(N)
...
def check_2(lst_obj):
    ...
    for i in range(len(lst_obj)):  # O(N) - линейная
        if lst_obj[i] in lst_obj[i + 1:]:  # O(N) - линейная
...
# итоговая сложность O(NlogN) - линейно-логарифмическая так как максимальная сложность O(NlogN)
#############################################################################################


# выражение ниже не отмечено знаком !!! зачем оно не понял.
for i in (50, 500, 1000, 5000, 1000):
    # Из 100000 чисел возьмем 'j' случайно выбранных
    # Всего 10 тыс. чисел
    lst = random.sample(range(-100000, 100000), i)
