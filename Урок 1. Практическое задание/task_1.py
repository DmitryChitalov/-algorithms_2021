...
def check_2(lst_obj):
    ...
    Сложность: max( O(n) * O(n) + O(1), O(n) * O(1) + O(1)) = O(n) * O(n) + O(1) = O(n * n + 1) = O(n^2).
    """
    for j in range(len(lst_obj)):          # O(n)
        if lst_obj[j] in lst_obj[j+1:]:    # O(n) или O(1) если нашли сразу
            return False                   # O(1)
    return True                            # O(1)
...
def check_3(lst_obj):
    ...
    Сложность: O( n + n * log(n) + n * 1) = O( n * log(n) )
    """
