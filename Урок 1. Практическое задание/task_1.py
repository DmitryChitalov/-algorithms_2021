...
def check_1(lst_obj):
    ...
    Сложность: O(2) - константная.
    """
    lst_to_set = set(lst_obj)  # O(1) - Константная
...
def check_2(lst_obj):
    ...
    for j in range(len(lst_obj)):          # O(N) - линейная
        if lst_obj[j] in lst_obj[j+1:]:    # O(N) - линейная
            return False                   # O(1) - константная - может никогда не будет выполнен.
...
def check_3(lst_obj):
    ...
    lst_copy.sort()                          # O(N Log N) - Логарифмическая
    for i in range(len(lst_obj) - 1):        # O(N) - Линейная
        if lst_copy[i] == lst_copy[i+1]:     # O(1) - Константная
            return False                     # (1) - Константная - может никогда не будет выполнен.
    return True                              # (1) - Константная
