...
def check_1(lst_obj):
    ...
    Сложность: O(len(lst_obg)), линейная.
    """
    lst_to_set = set(lst_obj)  # O(len(lst_obg))
...
def check_2(lst_obj):
    ...
    for j in range(len(lst_obj)):  # O(len(lst_obj))
        if lst_obj[j] in lst_obj[j + 1:]:  # O(j+1) + O(1)
            return False  # О(1)
    return True  # O(1)


# не очень поняла, надо везде писать n или быть более конкретной, как O(len(lst_obj))?
#############################################################################################
def check_3(lst_obj):
    """Функция должная вернуть True, если все элементы списка различаются.
    ...
    for i in range(len(lst_obj) - 1):  # O(n) + O(1)
        if lst_copy[i] == lst_copy[i + 1]:  # О(1) + О(1) + О(1)
            return False  # O(1)
    return True  # O(1)
