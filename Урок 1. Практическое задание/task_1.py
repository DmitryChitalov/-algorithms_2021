...
def check_1(lst_obj):
    ...
    Сложность: O(len(lst_obj)) + O(1) = O(len(lst_obj))?
    """
...
def check_2(lst_obj):
    ...
    for j in range(len(lst_obj)):          # O(N)
        if lst_obj[j] in lst_obj[j+1:]:    # O(1) + O(N) + (O(1) + O(N)) = O(N)?
            return False                   # O(1)
    return True                            # O(1)
...
def check_3(lst_obj):
    ...
    Сложность: O(len(lst_obj) + O(N log N) + O(N) + O(1) + O(1) + O(1) = O(N log N)
    """
    lst_copy = list(lst_obj)                 # O(len(lst_obj)
    lst_copy.sort()                          # O(N log N)
    for i in range(len(lst_obj) - 1):        # O(N) + O(1) = O(N)
        if lst_copy[i] == lst_copy[i+1]:     # O(1) + O(1) + O(1) + O(1) = O(1)
            return False                     # O(1)
    return True                              # O(1)
