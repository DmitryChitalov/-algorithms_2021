...
def check_1(lst_obj):
    ...
    Сложность: !!! Линейная O(N): O(N) + O(1), где N - длина списка lst_obj
    """
...
def check_2(lst_obj):
    ...
    Сложность: !!!
    Квадратичная (в худшем случае): O(N ** 2): O(N) * O(N) + O(1)
    Линейно-логарифмическая (по ощущениям - в среднем): O(N * Log N)
    где, N - длина lst_obj

    """
    for j in range(len(lst_obj)):          # !!! O(N)
        if lst_obj[j] in lst_obj[j+1:]:    # !!! от O(1) до O(N)
            return False                   # !!! O(1)
    return True                            # !!! O(1)
...
def check_3(lst_obj):
    ...
    Сложность: !!! Линейно-логарифмическая O(N Log N), где N - длина list_obj
    O(N * Log N) + 2 * O(N) + O(1)
    """
