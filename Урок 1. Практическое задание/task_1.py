...
def check_2(lst_obj):
    ...
    Сложность: O(n^2)
    """
    for j in range(len(lst_obj)):  # len: O(n)
        if lst_obj[j] in lst_obj[j + 1:]:  # срез: O(n) + in: O(n) + получение элемента: O(1) = O(n)
            return False  # O(1)
    return True  # O(1)
...
def check_3(lst_obj):
    ...
    Сложность: доминантная часть O(n log n)
    """
    lst_copy = list(lst_obj)  # создание списка: O(n)
    lst_copy.sort()  # сортировка: O(n log n)
    for i in range(len(lst_obj) - 1):  # len: O(1) + цикл: O(n) = O(n)
