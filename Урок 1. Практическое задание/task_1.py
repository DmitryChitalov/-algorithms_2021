...
def check_1(lst_obj):
    ...
    Сложность: O(n).
    """
    lst_to_set = set(lst_obj)  # O(n) - линейная, где n = len(lst_obj)
    return lst_to_set  # 0 - просто возвращаем указатель на множество
...
def check_2(lst_obj):
    ...
    Сложность: O(n^2).
    """
    for j in range(len(lst_obj)):          # O(n) - линейная, где n = len(lst_obj)
        if lst_obj[j] in lst_obj[j+1:]:    # 0(n) - линейная
            return False                   # 0 - просто возвращаем значение 0
    return True                            # 0 - просто возвращаем значение 1
...
def check_3(lst_obj):
    ...
    Сложность: O(n log n)
    """
    lst_copy = list(lst_obj)                 # O(n) - линейная, где n = len(lst_obj)
