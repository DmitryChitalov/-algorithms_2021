...
Примечание:
Прошу вас внимательно читать ТЗ и выполнять все пункты.
"""
...
#############################################################################################
def check_2(lst_obj):
    """Функция должна вернуть True, если все элементы списка различаются.
...
def check_3(lst_obj):
    ...
    lst_copy.sort()                          # O(N log N) - встроенная, линейно-логарифмическая
    for i in range(len(lst_obj) - 1):        # O(N) + O(N) + O(N) = O(N) - линейная
