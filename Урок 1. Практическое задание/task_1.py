...
def check_1(lst_obj):
    ...
    return lst_to_set  # O(1) - постоянное
...
def check_2(lst_obj):
    ...
    for j in range(len(lst_obj)):          # O(n) - линейное
        if lst_obj[j] in lst_obj[j+1:]:    # O(n) - линейное
            return False                   # O(1) - постоянное
    return True                            # O(1) - постоянное
...
def check_3(lst_obj):
    ...
    Сложность: O(n Log n)
    """
    lst_copy = list(lst_obj)                 # O(len(lst_obj) - зависит от длинны аргумента
    lst_copy.sort()                          # O(n Log n) - линейно-логорифмическое
    for i in range(len(lst_obj) - 1):        # O(n) - линейное
        if lst_copy[i] == lst_copy[i+1]:     # O(n) - линейное
            return False                     # O(1) - постоянное
    return True                              # O(1) - постоянное
