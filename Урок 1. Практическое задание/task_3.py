...
def get_richest_firms_2(income_dict):
    ...
    """
    all_firms = [(key, value) for key, value in income_dict.items()]  # O(N) + O(N)
    for pos in range(len(all_firms) - 1):  # O(N)
        for k in range(pos + 1, len(all_firms)):  # O(N)
            if all_firms[k][1] < all_firms[pos][1]:  # сравниваем значения из словарей
                all_firms[k], all_firms[pos] = all_firms[pos], all_firms[k]  # O(1)
    all_firms.reverse()  # O(N)
    return all_firms[:3]
