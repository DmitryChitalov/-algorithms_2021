...
# Вариант 3:
# реализуем хранение данных через словарь
company_dict = {
    'company_1': 2500000,
    'company_2': 50000,
    'company_3': 2500,
...
# Цикл с фиксированным количеством итераций O(1),
# Функция сортировка - O(n log n)
# Общая сложность O(n log n)
for i in range(3):
    print(sorted(company_dict.items(), key=lambda x: x[1], reverse=True)[i])

"""
Так как мы не можем предположить, что прибыли у всех компаний разные,
их нельзя использовать как ключи словаря, а встроенных методов быстрого поиска 
в словаре по значениям не существует. Что сводит на нет преимущества словаря для данной задачи.
В данном случае наилучшей асимптоматикой по времени выполнения будет обладать второй
вариант решения.
"""