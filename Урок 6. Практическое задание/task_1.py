...
def realization_1():
    class Worker:
        def __init__(self, sname: str, name: str, position: str, income: dict) -> None:
            self.name = name
            self.sname = sname
    ...
    class Position(Worker):
        def __init__(self, sname: str, name: str, position: str, income: dict) -> None:
            super().__init__(name, sname, position, income)
            pass

        def get_full_name(self) -> str:
            return f'{self.sname} {self.name}'
...
def realization_2():
    class Worker:
        __slots__ = ['name', 'sname', 'position', 'income']
...
'''
Оптимизация заключается в использовании слотов, тем самым достигается экономия памяти
'''
...
array_data = [2, 2, 2, 7, 23, 1, 44, 44, 3, 2, 10, 7, 4, 11]
...
print(timeit('calc_old(array_data)', globals=globals(), number=1000000))
print(timeit('calc_new(array_data)', globals=globals(), number=1000000))

'''
4.115004150997265
3.6450431589983054

Оптимизация достигается за счет использование lc предназначенного для более быстрого выполнения
