...
# Например, число «19 ^ 3 = 6859» будем включать в сумму, так как 6 + 8 + 5 + 9 =
# 28 – делится нацело на 7
@decor
def old_version(num):
...
def count_even_odd_numbs(number, even=0, odd=0):
    """Ф-я возвращает количество четных и нечетных
    цифр  числе"""
    if number == 0:
        return even, odd
    if number % 10 % 2 == 0:
...
@decor
def main_f(num):
    """Выводит результат ф-ии count_even_odd_numbs() с профилированием"""
...
print(f'Четных и нечетных чисел: '
      f'{count_even_odd_numbs2(6655523002)}')  # memory: 0.0, time: 0.10012310000000002
...
print(asizeof.asizeof(raw_date))  # 416
...
print(asizeof.asizeof(raw_date))  # 152
...
# экономия памяти, но работает это только на
