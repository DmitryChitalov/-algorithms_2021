...
"""
def reverse():
    n = input('Введите число, которое требуется перевернуть: ')
    n_list = list(n)
    n_list.reverse()
    n2 = ''.join(n_list)
    print('Перевернутое число:', n2)

reverse() # C рекурсией не понял как , только с разбора уже

def reverse2(n):
    return str(n) if n < 10 else str(n % 10) + reverse2(n // 10)

n = int(input('Введите число, которое требуется перевернуть: '))
print('Перевернутое число:', reverse2(n))