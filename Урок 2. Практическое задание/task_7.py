...
def check_func(n):
    if n == 1:
        ...
        return (n + check_func(n-1))


n = int(input('Введите число: '))
print(f'Для числа {n}, результат проверки равенства 1+2+...+n = n(n+1)/2: \n'
      f'леввя часть {check_func(n)} = правая часть {((n* (n + 1)) / 2)}\n'
      f'Результат {(check_func(n)) == (int(((n* (n + 1)) / 2)))}')
