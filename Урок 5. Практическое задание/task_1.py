...
def sort_firm_avg(firm_list:list,avg:int)->[list,list]:
    '''
    Сортировка по среднегодовому доходу
    '''
    up_firm = list()
    down_firm = list()
    for f in firm_list:
        if f.avg > avg:
            up_firm.append([f.name,f.avg])
        elif f.avg <= avg:
            down_firm.append([f.name,f.avg])

    return up_firm,down_firm


firm_list = get_firm()
avg = all_avg(firm_list)
up_list,dwn_list = sort_firm_avg(firm_list,avg)

print(f"Средняя годовая прибыль всех предприятий: {avg}")
print(f"Предприятия, с прибылью выше среднего значения: {up_list}")
print(f"Предприятия, с прибылью ниже среднего значения: {dwn_list}")
