...
def firms_profit(num):
    ...
    for j in arr:
        summa += j.Profit
    summa = summa / num
    print(f'Средняя годовая прибыль всех предприятий равна: {summa}')
    for k in arr:
        print(
            f"Предприятия, с прибылью {'выше' if k.Profit > summa else 'ниже'}"
            f" среднего значения: {k.Name}")


print(firms_profit(num_of_companies))
