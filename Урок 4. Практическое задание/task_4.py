...
используя ф-ю max решение будет в одну строку
'''


# ???
def func_4():
    num = max(array1, key=lambda i: array1.count(i))
    m = array1.count(num)
    return f'Чаще всего встречается число {num}, ' \
           f'оно появилось в массиве {m} раз(а)'


array1 = [1, 3, 1, 3, 4, 5, 1]
print(func_4())
t4 = Timer("func_4()", "from __main__ import func_4, array1")
print(f'Суммарное время 10000 запусков func_4 {t4.timeit(number=10000) * 1000:.3f} мсек')
