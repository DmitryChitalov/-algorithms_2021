...
#  Предлагаемая оптимизация: Методы insert_right и insert_left делаем защищенными, а пользователю предоставляем
#  для добавления узлов метод insert_child, который принимает на вход значение узла, определяет основную ветвь
#  (правую или левую) и далее вызывает методы insert_right и insert_left, которые рекурсивно
#  вызывают друг друга в процессе определения места в дереве в зависимости от значения нового узла и уже ранее
#  добавленных узлов дерева. Для проверки корректности построения дерева добавлен атрибут path, куда пишется путь
#  размещения узла. В интерфейс дообавлен еще метод поиска узла по значению
