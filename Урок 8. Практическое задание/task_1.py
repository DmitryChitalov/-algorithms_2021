...
def get_tree(string):
    ...
    if len(string_count) <= 1:
        node = HuffmanString(None)

        if len(string_count) == 1:
            node.left = HuffmanString([key for key in string_count][0])
            node.right = HuffmanString(None)
    ...
    while len(string_count) != 1:
        ...
        if isinstance(spam[0][0], str):
            node.left = HuffmanString(spam[0][0])

        else:
            node.left = spam[0][0]

        if isinstance(spam[1][0], str):
            node.right = HuffmanString(spam[1][0])
