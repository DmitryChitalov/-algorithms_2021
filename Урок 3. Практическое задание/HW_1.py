...
stop = time()
print(f'Время выполнения: {stop - start}')  # Время выполнения: 0.0009970664978027344
start = time()
...
"""
ИТОГ: 
1)чем сложнее функция, тем дольше она выполняется, что доказывает правильно определенную О нотацию
2)словарь наполняется быстрее списка при использовании одинакового метода наполнения
"""
