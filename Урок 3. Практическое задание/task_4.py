...
if __name__ == '__main__':
    # Можно ввести модель при которой происходит разбор адреса и сохранение в кэше самого сайта или его страниц,
    # а не всей совокупности поисковых условий, но пока это избыточная задача.

    #  Добавляем единичную запись, проверяем число записей. 1 шт.
    UrlCache().check_url('https://gb.ru/lessons/145595/homework')
    UrlCache().get_len_cache()

    #  Смотрим что есть в кэше. Получаем таблицу. Подтверждаем наличие единицы.
    UrlCache().show_cache()
...
#     Интерфейс функционирует корректно. Хэш таблица создается, хэш "солится". Реализовано с ООП.
