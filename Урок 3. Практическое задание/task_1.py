"""
Задание 1.

Реализуйте свои пользовательские функции, в которых реализуйте:

a) заполнение списка и словаря,
   сделайте замеры и сделайте выводы, что выполняется быстрее и почему
b) выполните набор операций и со списком, и со словарем,
   сделайте замеры и сделайте выводы, что и где выполняется быстрее и почему

Подсказка: для замеров воспользуйтесь модулем time (см. примеры урока 1)

Примечание: eсли вы уже знаете, что такое декоратор и как его реализовать,
то реализуйте ф-цию-декоратор для подсчета времени работы ваших пользовательских функций
И примените ее к своим функциям!

Прошу вас внимательно читать ТЗ и не забыть выполнить все пункты.
"""
from time import time


def time_work(fun):
    def g(r=1000000):
        t1 = time()
        fun(r)
        t2 = time()
        time_out = t2 - t1
        return time_out

    return g


@time_work
def list_1(n):
    return [x for x in range(n)]


@time_work
def dict_1(n):
    return {x: x for x in range(n)}


print(f"время создания списка: {list_1(1)}")
print(f"время создания словаря: {dict_1(1)}")
"""Вывод: Словари оказались медленее списка. Так как, словари, выполняет два действия, 
записыват значеие и присваивает к этому
значению ключ."""


@time_work
def list_2(n):
    arr = [x for x in range(n)]
    del arr[n - 1]
    arr2 = n - 1 in arr
    arr.pop()
    arr.append('hell')
    len(arr)
    return arr


@time_work
def dict_2(n):
    arr = {x: x for x in range(n)}
    del arr[n - 1]
    arr2 = n - 1 in arr
    arr.popitem()
    arr.update({'hell': 'hell'})
    len(arr)
    return arr


print(f"время создания 2-го списка: {list_2(1)}")
print(f"время создания 2-го словаря: {dict_2(1)}")
"""Если не принимать во внимание время создания словаря и списка, то по скорости выполнение задач выйграл словарь. 
Так как он сразу находит то что нужна по ключам не тратя время на перебор элементов в одличии от словаря"""
