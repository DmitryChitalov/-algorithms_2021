...
print(getsizeof(my_dict[0]) / getsizeof(my_list[0]))
"""
Как мы видим, объем памяти, занимаемый словарем, в несколько раз превышает память, требующуюся списку, вероятно, это
приводит к тому, что Python чаще имзеняет выделенный размер структуры.
Также создание словаря подразумевает создание хэш-таблицы, вычисление хэшей и её заполнение, в то время как,
чтобы добавить значение в конец списка дополнительные расчеты производить не нужно.
"""
