...
@time_decorator
def gen_list(n):
    """Наполняем список с помощью генератора"""
...
# Как видим список наполняется быстрее словоря а генераторы работают быстрее простого цикла
# И генератор словоря почти так же быстр как наполнение списка циклом
