...
a) заполнение списка и словаря,
   ...
   У этих ф-ций может быть разная сложность. Поэтому время заполнения списка и словаря может как совпадать, так и отлич
   аться.
...
# длина
'''скорость выше у словаря. Наверное потомучто у словаря есть обьект Py_size_t ma_used -количество элементов словаря'''
t1 = time.time()
print(len(d))
...
# удаление
'''скорость выше у словаряю. Потомучто есть обьект Py_size_t ma_used - количество элементов словаря, 
